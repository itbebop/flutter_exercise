...
class PostRepository {
  ...
  Future<int> deleteById(int id) async {
    ...
    return cmRespDto.code ?? -1; // code가 null이면 -1을 넣는다
  ...
  Future<Post> findById(int id) async { // Future와 async는 세트
    ...
    if (cmRespDto.code == 1) {
      Post post = Post.fromJson(cmRespDto.data); // 한건이라 list로 바꿔줄 필요 없음
      return post;
    } else {
      return Post();
    }
  ...
  Future<List<Post>> findAll() async {
    ...
    if (cmRespDto.code == 1) { // 통신성공
      List<dynamic> temp = cmRespDto.data; // map으로 받으려는데 dynamic타입이라서 컴파일될 때 받을 타입을 알 수 있고 지금은 data.map을 쓰지 못함
